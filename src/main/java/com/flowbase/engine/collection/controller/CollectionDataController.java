...
class CollectionDataController {
    ...
    @PostMapping
    public ResponseEntity<CollectionDocumentResponse> createDocument(@PathVariable String collectionId,
                                                                     @RequestBody Map<String, Object> payload) {
        CollectionDocument document = this.collectionDataService.insertDocument(collectionId, payload);
        return ResponseEntity.status(HttpStatus.CREATED).body(this.mapToResponse(document));
    }
    
    @GetMapping("/{documentId}")
    public ResponseEntity<CollectionDocumentResponse> getDocument(@PathVariable String collectionId,
    ...
    public ResponseEntity<List<CollectionDocumentResponse>> listDocuments(@PathVariable String collectionId) {
        List<CollectionDocument> collectionDocuments =
                this.collectionDataService.findDocumentsByCollection(collectionId);
        return ResponseEntity.ok(collectionDocuments.stream().map(this::mapToResponse).toList());
    }
    
    @PatchMapping("/{documentId}")
    public ResponseEntity<CollectionDocumentResponse> updateDocument(@PathVariable String collectionId,
                                                                     @PathVariable String documentId, @RequestBody Map<String, Object> payload) {
    ...
    private CollectionDocumentResponse mapToResponse(CollectionDocument document) {
        return new CollectionDocumentResponse(document.id(), document.collectionId(), document.data(),
